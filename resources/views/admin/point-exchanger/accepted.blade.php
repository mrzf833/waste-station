            ...
            <div class="card-content">
                <div class="card-body">
                    @component('components.datatable',['datas' => ['Client', 'Penerima', 'Barang', 'point price', 'alamat', 'kode pos','diterima','status']])
                        @slot('id')
                            point_exchanger_accepted
                        @endslot
                    @endcomponent
                </div>
            </div>
        </div>
        @component('components.modal-form',['id_modal' => 'modal-delete-point_exchanger_accepted', 'text_title' => 'Delete point_exchanger_accepted','action' => ''])
            @slot('content_modal')
                Apakah anda yakin mengahapus point_exchanger_accepted : <span id="name_point_exchanger_accepted_delete"></span>
            @endslot
            @slot('method')
                @method('DELETE')
            @endslot
...
<script>
    var table_point_exchanger_accepted = $('#point_exchanger_accepted').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('point_exchanger.datatable.accepted') }}",
            },
            columns: [
                {
                    data: 'email',
