            ...
            <div class="card-content">
                <div class="card-body">
                    @component('components.datatable',['datas' => ['Client', 'Penerima', 'Barang', 'point price', 'alamat', 'kode pos','diterima','status']])
                        @slot('id')
                            point_exchanger_rejected
                        @endslot
                    @endcomponent
                </div>
            </div>
        </div>
        @component('components.modal-form',['id_modal' => 'modal-delete-point_exchanger_rejected', 'text_title' => 'Delete point_exchanger_rejected','action' => ''])
            @slot('content_modal')
                Apakah anda yakin mengahapus point_exchanger_rejected : <span id="name_point_exchanger_rejected_delete"></span>
            @endslot
            @slot('method')
                @method('DELETE')
            @endslot
...
<script>
    var table_point_exchanger_rejected = $('#point_exchanger_rejected').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('point_exchanger.datatable.rejected') }}",
            },
            columns: [
                {
                    data: 'email',
